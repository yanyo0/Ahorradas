...
let category = [
  ...
  { id: 4, title: "Educación" },
  { id: 5, title: "Transporte" },
  { id: 6, title: "Trabajo" },
];
...
window.addEventListener("load", () => {
  ...
  let valueEditDescriptionOperation,
    valueEditAmountOperation,
    valueEditTypeOperation,
    valueEditCategoryOperation,
    valueEditDateOperation;
  ...
  const setLocalStorege = (key, array) =>
  ...
  const filterDateOperations = (valueDate) => {
    if (valueDate !== "") {
      filteredOut = filteredOut.filter(
        (elem) =>
          Number(year(elem.date)) >= Number(year(tranformDate(valueDate)))
      );
      filteredOut = filteredOut.filter(
        (elem) =>
          Number(month(elem.date)) >= Number(month(tranformDate(valueDate)))
      );
      filteredOut = filteredOut.filter(
        (elem) => Number(day(elem.date)) >= Number(day(tranformDate(valueDate)))
      );
    }
    return filteredOut;
  };
  ...
  const paintOperation = (array) => {
    ...
    if (array.length === 0) {
      ...
      array.forEach((elem) => {
        $boxOperations.innerHTML += `
                            <tr id="${elem.id}">
                            <th class="flex justify-start mt-5 sm:mt-0">${
                              elem.description
                            }</th>
                            <th>${elem.category}</th>
                            <th class="hidden sm:flex justify-center">${
                              elem.date
                            }</th>
                            <th class="${
                              elem.type === "ganancia"
  ...
  const addNewOperation = () => {
    operations.push({
      ...
      type: typeOperation(valueType),
      category: valueCategory,
      date: tranformDate(valueDate),
    });
  };
    ...
    $$btnEditOperation.forEach((icon) =>
      icon.addEventListener("click", (e) => {
        ...
        select = operations.filter(
          (elem) => elem.id === icon.parentNode.parentNode.parentNode.id
        );
        getDataInputEditOperation();
      })
    );

    $$btnCleanOperation.forEach((icon) =>
      icon.addEventListener("click", (e) => {
  ...
  const valueAddCategory = (value) => {
    let serchCategory = category.filter(
      (elem) => elem.title.toLocaleLowerCase() === value.toLocaleLowerCase()
    );
  ...
  const valueEditCategory = (value) => {
    let serchCategory = category.filter(
      (elem) =>
        elem.title.toLowerCase() === value.toLowerCase() &&
        elem.id !== select[0].id
    );
    errorsTitleCategory = false;
  ...
  const addEventIconEditCategory = () => {
    ...
    $$btnEditCategoryItems.forEach((icon) =>
      icon.addEventListener("click", (e) => {
        ...
        select = category.filter(
          (elem) => elem.id === Number(icon.parentNode.parentNode.id)
        );
        $editNameCategory.value = select[0].title;
      })
    );

    $$btnCleanCategoryItems.forEach((icon) =>
      icon.addEventListener("click", (e) => {
    ...
    for (let i = 0; i < category.length; i++) {
      if (suma < totalPositiveBalance(divisionCategory(category[i].title))) {
        ...
        categoryMoreGain = {
          ...
          amount: totalPositiveBalance(divisionCategory(category[i].title)),
        };
      }
    }
  ...
  const categoryHigherExpense = () => {
    ...
    for (let i = 0; i < category.length; i++) {
      if (suma < totalNegativeBalance(divisionCategory(category[i].title))) {
        ...
        categoryLessExpense = {
          category: category[i].title,
          amount: totalNegativeBalance(divisionCategory(category[i].title)),
        };
      }
    }
  ...
  const categoryMoreBalance = () => {
    ...
    for (let i = 0; i < category.length; i++) {
      if (balance(divisionCategory(category[i].title)) > suma) {
        ...
        moreBalance = {
          category: category[i].title,
          amount: balance(divisionCategory(category[i].title)),
        };
      }
    }
    return moreBalance;
  ...
  const balancesMonth = () => {
    ...
    for (let i = 0; i < arrayYearsOperation.length; i++) {
      ...
      for (const num of numMonths) {
        months[num].operation = arrayYearsOperation[i].operation.filter(
          (elem) => month(elem.date) == num
        );
      }
  ...
  const monthHigherGain = () => {
    ...
    for (let i = 0; i < operationsDivision.length; i++) {
      for (let ind = 1; ind < operationsDivision[i].meses.length; ind++) {
        if (
          totalPositiveBalance(operationsDivision[i].meses[ind].operation) >
          suma
        ) {
          suma = totalPositiveBalance(
            operationsDivision[i].meses[ind].operation
          );
          higherGain = {
            date: `${ind.toString().length === 1 ? "0" + ind : ind}-${
              operationsDivision[i].anio
            }`,
  ...
  const monthHigherExpense = () => {
    ...
    for (let i = 0; i < operationsDivision.length; i++) {
      for (let ind = 1; ind < operationsDivision[i].meses.length; ind++) {
        if (
          totalNegativeBalance(operationsDivision[i].meses[ind].operation) >
          suma
        ) {
          suma = totalNegativeBalance(
            operationsDivision[i].meses[ind].operation
          );
          higherExpense = {
            date: `${ind.toString().length === 1 ? "0" + ind : ind}-${
  ...
  const paintSectionCategorySummary = () => {
    if (operations.length !== 0) {
      if (categoryHigherGain() !== undefined) {
        ...
        $categoryHigherGainAmount.innerText = `$ ${
          categoryHigherGain().amount
        }`;
      } else {
      ...
      if (categoryHigherExpense() !== undefined) {
        ...
        $categoryHigherExpenseAmount.innerText = `$ ${
          categoryHigherExpense().amount
        }`;
      } else {
      ...
      if (categoryMoreBalance() !== undefined) {
        ...
        $categoryMoreBalanceAmount.innerText = `$ ${
          categoryMoreBalance().amount
        }`;
      } else {
      ...
      if (monthHigherExpense() !== undefined) {
        ...
        $monthHigherExpenseAmount.innerText = `$ ${
          monthHigherExpense().amount
        }`;
      }
    }
  };
    ...
    category.forEach((elem) => {
      $totalCategoryBox.innerHTML += `
        <div class="flex flex-row justify-between">
                        ...
                        <p class="w-1/4 flex justify-end text-lime-600">+$ ${
                          totalCategory(elem.title).positive
                        }</p>
                        <p class="w-1/4 flex justify-end text-red-600">-$ ${
                          totalCategory(elem.title).negative
                        }</p>
                        <p class="w-1/4 flex justify-end">$${
                          totalCategory(elem.title).balance
                        }</p>
                    </div>
        `;
    });
  ...
  const sumOperationMonth = () => {
    ...
    for (let i = 0; i < operationsDivision.length; i++) {
      for (let a = 1; a < operationsDivision[i].meses.length; a++) {
        date = `${a.toString().length === 1 ? "0" + a : a}-${
          operationsDivision[i].anio
        }`;
        totalPositive = totalPositiveBalance(
          operationsDivision[i].meses[a].operation
        );
        totalNegative = totalNegativeBalance(
          operationsDivision[i].meses[a].operation
        );
